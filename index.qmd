...
Я не журналист, просто люблю находить какие-то смыслы с помощью данных. В этот раз вышло немного больше чем в 20-ом. Собственно статья по моим данным вышла в [Новой газете](https://novayagazeta.eu/articles/2024/03/20/vosstanie-relokantov?fbclid=IwAR01A66cysVn84QduPZMTBjP3c1cIEfAVLW_drlLD7OGn7bNYmAMvk7sNck "Восстание релокантов."), за что большое спасибо Алесе Соколовой. Подписывайтесь на [её канал](https://t.me/research_for_democracy "Ненастоящий сошиал ресерч"). Если есть какие-то мысли пишите мне в телеграм @vearlen.
...
#   
  ...
  g1 <- total_voters %>% 
    ...
    geom_text(aes(label=total,y=total*0.84),vjust = 1.5,color='grey99',size=4)+
    coord_flip()+
    theme_cowplot()+
    theme_minimal_hgrid()+
...
RU_Pro_flag <- RU04_24 %>% 
  ...
  mutate(ru_pro = case_when(
    str_detect(Location,
  'Миноб|ФСБ|батальон|ОКРМС|штаб|в/ч|Матросский|офицеров|ОШК|ДОФ|КСПМ|Батальон|Клуб 1472')~'военные и пр.',
    str_detect(country,'Молда|Абха|Осет')~ "военные и пр.",
    TRUE ~"гражданские")) 
...
RU04_24_PM %>% 
  ...
  facet_grid(year~.)+
  labs(x="процент за Путина, %", y= "голосов за Путина",fill="")+
...
У Важных историй получилось, что процент военных и квазигосударств [составляет 46%](https://istories.media/news/2024/03/18/bolee-46-golosov-za-putina-na-zarubezhnikh-territoriyakh-polucheno-v-podkontrolnikh-rossii-stranakh-i-voinskikh-chastyakh/), у меня вышло в районе 37% в 2024.

```{r proportion of militaries}
RU04_24_PM %>%
  mutate(Location = str_trim(Location)) %>%
  left_join(RU_Pro_flag,by=join_by(UIK,year,Location,en_country)) %>%
  ...
  mutate(Y_N_ratio = `военные и пр.`/(`военные и пр.`+`гражданские`)) %>% 
  ggplot()+
  geom_col(aes(x=year,y=Y_N_ratio))+
  geom_text(aes(x=year,label=paste0(round(Y_N_ratio*100,0),"%"),y=Y_N_ratio*0.92),vjust = 1.5,color='grey99')+
  theme_minimal()+
  scale_y_continuous(labels = scales::percent_format(accuracy = 1))+
  labs(title="Доля военных",x="",y="")+
  scale_x_continuous(breaks = c(2004,2008,2012,2018,2020,2024))
...
g1 <- RU04_24_PM %>% 
  ...
  ggplot(aes(x=people,y=rat, 
             ...
             text = paste0(en_country,'<br>кол-во: ',people,"<br>%: ",rat)))+
  geom_point(alpha = 0.6, size=2)+
  scale_x_log10()+
  theme_minimal()+
  labs(y="процент за Путина, %",x = "кол-во людей, логарифмированная шкала",color="",title = "Кол-во людей на участке и процент за Путина")
